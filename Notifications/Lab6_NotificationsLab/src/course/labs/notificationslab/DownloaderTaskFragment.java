...
//import course.labs.asynctasklab.DownloaderTaskFragment.DownloaderTask;
...
public class DownloaderTaskFragment extends Fragment {
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        ArrayList<Integer> IDs = bundel.getIntegerArrayList("friends"); 
	...
	public class DownloaderTask extends AsyncTask<ArrayList<Integer>, Void, String[]>{
		...
		protected String[] doInBackground(ArrayList<Integer>... IDs){			
			Integer[] resourceIDS = new Integer[IDs[0].size()];		
			for (int i=0;i<IDs[0].size();i++){
				resourceIDS[i] = IDs[0].get(i);
			}
		...
		@Override
		protected void onPostExecute(String[] newFeeds){
			super.onPostExecute(newFeeds);
			Log.i(TAG, "abcde");
			if (mCallback != null){
				mCallback.notifyDataRefreshed(newFeeds);
				Log.i(TAG, "pass");}
			else{
				Toast.makeText(mContext, R.string.notification_sent_string, Toast.LENGTH_LONG).show();
				Log.i(TAG, "another pass");
			}
					...
					new BroadcastReceiver() {
						...
						public void onReceive(Context context, Intent intent) {
							...
							if (getResultCode() != MainActivity.IS_ALIVE) {
								...
								PendingIntent pending = PendingIntent.getActivity(context, getResultCode(), 
